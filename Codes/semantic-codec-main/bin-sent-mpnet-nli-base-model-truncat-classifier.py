...
base_full_dim = 768
base_trunc_dim = 1 # sets the number of dimensions that stella embeddings will be truncated with ([1,2,4,8,16,32,64,128,256,512])
...
class ReferenceModel(torch.nn.Module):
    def __init__(self, model, device=None, base_trunc_dim=None):
        ...
        self.base_trunc_dim = base_trunc_dim
    ...
    def encode(self,
        ...
        for embed in embeddings:
            embed = embed[0:self.base_trunc_dim] # gets only the 'base_trunc_dim' first elements of each embed
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-folder", help="output folder for benchmark results", default="./benchmark-results/tomaarsen-mpnet-base-nli/stanfordnlp-sst2/Truncated")
    ...
    reference_model: ReferenceModel = ReferenceModel(model=SentenceTransformer(model_name, device=current_device, trust_remote_code=True), device=current_device, base_trunc_dim=base_trunc_dim)
    ...
    results_path = os.path.join(output_folder_path, f'{model_name}_{base_trunc_dim}_dims', 'original')
    ...
    if quantizer_name=="ProductQuantizer":
        for n_subvecs in vec_n_subvecs:
            ...
            evaluation = MTEB(tasks=tasks)
            model_name = make_safe_path_name(model_name)
            results_path = os.path.join(output_folder_path, f'{model_name}_{base_trunc_dim}_{base_full_dim}_dims', 'PQ_NaiveBinarizer', f'IdentityChannel', f'{n_subvecs}')
        ...
        logging.info(f"evaluating model on '{channel_name}' and using '{base_trunc_dim}' as the truncated dimension")
        channel = IdentityChannel()
        study_model = StudyModel(model=reference_model, dataset_name=dataset_name, quantizer_name=quantizer_name, subcodebooks=subcodebooks, quantizer=quantizer, binarizer=binarizer, channel=channel, device=current_device)
        evaluation = MTEB(tasks=tasks)
        model_name = make_safe_path_name(model_name)
        results_path = os.path.join(output_folder_path, f'{model_name}_{base_trunc_dim}_dims', 'Truncated', f'IdentityChannel')
        results = evaluation.run(study_model, reference_model=reference_model, overwrite_results=True, output_folder=results_path)
        logging.info(f"results: {results}")
        results = results[0].to_dict()
        for task in tasks:
    ...
    if quantizer_name=="ProductQuantizer":
        results_df.to_json(os.path.join(output_folder_path, f'{model_name}_{base_trunc_dim}_dims', "PQ_NaiveBinarizer_IdChannel.json"))
    else:
        results_df.to_json(os.path.join(output_folder_path, f'{model_name}_{base_trunc_dim}_dims', "Truncated_IdChannel.json"))