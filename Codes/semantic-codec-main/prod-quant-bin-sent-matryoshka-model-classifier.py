...
quantizer_name = "ProductQuantizer"
dataset_name = "stanfordnlp/sst2" # possible options: "banking77", "stanfordnlp/sst2"
matryoshka_dim = 768 # sets the number of dimensions that matryoshka embeddings will be truncated with
vec_n_subvecs = [1,2,4,8,16,32,64,128,256,768] # vector specifying the multiple number of subvectors to be created during the Product Quantization process
n_clusterings = 100 # number of different clustering executions to build PQ subcodebooks


###### Setting up a reference model which computes the embeddings ######
class ReferenceModel(torch.nn.Module):
    def __init__(self, model, device=None, is_StudyModel_train=False):
        ...
        self.is_StudyModel_train = is_StudyModel_train
    ...
    def encode(self,
        ...
        if self.is_StudyModel_train==False:
...
class StudyModel(torch.nn.Module):
    ...
    def encode(self,
        ...
        if isinstance(embeddings, list) and self.dataset_name=="stanfordnlp/sst2" and self.activate_quantization==False:
            # logging.info('binarizing embeddings for the data on SST2')
        ...
        elif isinstance(embeddings, torch.Tensor) and self.dataset_name=="stanfordnlp/sst2" and self.activate_quantization==False:
            # logging.info('binarizing embeddings for the data on SST2')
            ...
            if self.quantizer_name=="ProductQuantizer":
                ...
                #embeddings_tensor: torch.Tensor = torch.Tensor(embeddings)
                embeddings_tensor = embeddings
                quantized_embeddings = self.quantizer.to_indices(embeddings_tensor)
                # logging.info('binarizing embeddings #3')
                binarized_embeddings = self.binarizer.encode(quantized_embeddings)
                # logging.info('channeling indices #3')
                rx_embeddings = self.channel.apply(binarized_embeddings)
                ...
                rx_indices = self.binarizer.decode(rx_embeddings)
                # logging.info('reconstructing received embeddings #3')
                rx_quantized_embeddings = self.quantizer.from_indices(rx_indices)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-folder", help="output folder for benchmark results", default="./benchmark-results/tomaarsen-mpnet-base-nli-matryoshka/stanfordnlp-sst2/PQ_NaiveBinarizer")
    ...
    reference_model: ReferenceModel = ReferenceModel(model=SentenceTransformer(model_name, truncate_dim=matryoshka_dim, device=current_device, trust_remote_code=True), device=current_device, is_StudyModel_train=False)
    ...
    file_path_train_embeds = f"./embeddings/tomaarsen-mpnet-base-nli-matryoshka/stanford-sst2_tomaarsen-mpnet-base-nli-matryoshka_train-embeds_not-normalized_{matryoshka_dim}_dims.csv"
    ...
    results_path = os.path.join(output_folder_path, f'{model_name}_{matryoshka_dim}_dims', 'original_new')
    ...
    binarizer = NaiveBinarizer(bits=1)
    scores = {task:[] for task in tasks}
    
    if quantizer_name=="ProductQuantizer":
        for idx_clustering in range(n_clusterings):
            for n_subvecs in vec_n_subvecs:
                ...
                evaluation = MTEB(tasks=tasks)
                model_name = make_safe_path_name(model_name)
                results_path = os.path.join(output_folder_path, f'{model_name}_{matryoshka_dim}_dims', f'Clustering_new_{idx_clustering}', 'PQ_NaiveBinarizer', f'IdentityChannel', f'{n_subvecs}')
                ...
                for task in tasks:
                    scores[task].append(results['scores']['validation'][0]['main_score'])
        
            results_df = pd.DataFrame(scores, index=vec_n_subvecs)
            results_df.to_json(os.path.join(output_folder_path, f'{model_name}_{matryoshka_dim}_dims', f'Clustering_new_{idx_clustering}', "PQ_NaiveBinarizer_IdChannel.json"))
            for task in tasks:
                scores[task] = []
    else:
        logging.info(f"evaluating model on '{channel_name}' and using '{quantizer_name}' as quantization")
        channel = IdentityChannel()
        study_model = StudyModel(model=reference_model, dataset_name=dataset_name, quantizer_name=quantizer_name, subcodebooks=subcodebooks, quantizer=quantizer, binarizer=binarizer, channel=channel, device=current_device)
        evaluation = MTEB(tasks=tasks)
        results_path = os.path.join(output_folder_path, make_safe_path_name(model_name), 'ieee754', f'IdentityChannel')
        results = evaluation.run(study_model, reference_model=reference_model, overwrite_results=False, output_folder=results_path)
        logging.info(f"results: {results}")
        results = results[0].to_dict()
        for task in tasks:
            scores[task].append(results['scores']['validation'][0]['main_score'])
        ...
        results_df.to_json(os.path.join(output_folder_path, make_safe_path_name(model_name), f"{quantizer_name}_IdChannel_SingleCase.json"))