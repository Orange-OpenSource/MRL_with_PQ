...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", help="model used to generate sentence embeddings *(tomaarsen/mpnet-base-nli)", default="tomaarsen/mpnet-base-nli", type=str)
    parser.add_argument("--output-folder", help="output folder for benchmark results", default="./benchmark-results/tomaarsen-mpnet-base-nli/stanfordnlp-sst2/PQ_NaiveBinarizer")
    ...
    reference_model: ReferenceModel = ReferenceModel(model=SentenceTransformer(model_name, device=current_device, trust_remote_code=True), device=current_device, is_StudyModel_train=False)
    ...
    file_path_train_embeds = f"./embeddings/tomaarsen-mpnet-base-nli/stanford-sst2_tomaarsen-mpnet-base-nli_train-embeds_not-normalized_{base_dim}_dims.csv"
    ...
    results_path = os.path.join(output_folder_path, f'{model_name}_{base_dim}_dims', 'original_new')
    ...
    if quantizer_name=="ProductQuantizer":
        for idx_clustering in range(n_clusterings):
            for n_subvecs in vec_n_subvecs:
                ...
                evaluation = MTEB(tasks=tasks)
                model_name = make_safe_path_name(model_name)
                results_path = os.path.join(output_folder_path, f'{model_name}_{base_dim}_dims', f'Clustering_new_{idx_clustering}', 'PQ_NaiveBinarizer', f'IdentityChannel', f'{n_subvecs}')
            ...
            results_df.to_json(os.path.join(output_folder_path, f'{model_name}_{base_dim}_dims', f'Clustering_new_{idx_clustering}', "PQ_NaiveBinarizer_IdChannel.json"))
            for task in tasks:
                scores[task] = []
    else:
